...
data = pd.read_csv('Predict_t_stage.csv')

#Drop the rows where 't_stage' has no value (i.e. Not Available)
data = data[~data['t_stage'].isin(['[Not Available]'])]

#Drop non-required columns
data.drop(['psa','gleason_score'], axis=1, inplace=True)

X = np.array(data.drop(['t_stage'],1))

#Target 't_stage'
Y = np.array(data['t_stage'])
...
lda = LinearDiscriminantAnalysis(n_components =7,solver='svd', shrinkage=None)
...
colors = ['g','r','c','b','k','m','yellow','orchid','fuchsia','lightcoral','g']
